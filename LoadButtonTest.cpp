...
class LoadButtonEdit : public LoadButton {
    public:
    //added variables to store info read from file in load game function for testing
    int** array;
    int rows;
    int columns;
    int time;
...
class LoadButtonTest {
    public:
    void tests() {
        std::cout << "Test 1: 7 x 7 array (as saved and loaded board have extra 2 tiles of padding)" << std::endl;
        ...
        std::cout << "Test 2: 28 x 28 array (as saved and loaded board have extra 2 tiles of padding)" << std::endl;
    ...
    private:
    void loadGameTest(std::string csv, int** _array, int _rows, int _columns, int _time) {
...
//set info in the csv file
//read out using load game
//check it matches 

//also look at if timer starts at correct time